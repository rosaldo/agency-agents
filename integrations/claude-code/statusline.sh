...
# Subscription quotas (absent on API-key billing)
five=$(jq -r '.rate_limits.five_hour.used_percentage // empty' <<<"$input" | cut -d. -f1)
if [[ -n "$five" ]]; then
  reset=$(jq -r '.rate_limits.five_hour.resets_at // empty' <<<"$input")
  [[ -n "$reset" ]] && reset=" ↻$(date -d "@$reset" +%H:%M 2>/dev/null || date -r "$reset" +%H:%M)"
  out+=$(printf ' | %s5h %d%%\033[0m%s' "$(color "$five")" "$five" "$reset")
fi
week=$(jq -r '.rate_limits.seven_day.used_percentage // empty' <<<"$input" | cut -d. -f1)
[[ -n "$week" ]] && out+=$(printf ' | %s7d %d%%\033[0m' "$(color "$week")" "$week")
